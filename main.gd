...
func _join_server(ip: String, port: int) -> void:
	...
	if err:
		print("Failed to join server: ", error_string(err))
		return
...
func _move_player_scenes() -> void:
	...
	var offset = 0.0
	for child in $Players.get_children():
		if multiplayer.get_unique_id() == child.player:
			continue
...
func _on_game_started() -> void:
	for child in $Players.get_children():
		if child.player == multiplayer.get_unique_id():
			_game = child
			_game.food_eaten.connect(_on_food_eaten)
			_game.died.connect(_on_snake_died)
			break
